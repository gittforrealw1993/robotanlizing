...
def add_swing_levels(df: pd.DataFrame, lookback: int = config.SWING_LOOKBACK) -> pd.DataFrame:
    """افزودن آخرین سوئینگ (سقف/کف) قبل از هر کندل.

    در ایندکس i، سوئینگ از N کندلِ قبلی (قبل از i) محاسبه می‌شود تا
    زمانی که سیگنال در کندل i صادر می‌شود، به کندل باز نشده اطلاع نداشته باشد.
    """
...
def add_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = add_swing_levels(df, lookback=config.SWING_LOOKBACK)
    return df
    ...
    for i in range(1, len(df)):
        ...
        if prev_9 <= prev_36 and curr_9 > curr_36:
            ...
            if close > curr_9 and close > curr_36:
                entry = close
                stop = df["swing_low"].iloc[i]
                if pd.notna(stop) and stop < entry:
                    risk = entry - stop
                    reward = risk * config.REWARD_RISK_MULT
                    df.at[df.index[i], "entry"] = entry
                    df.at[df.index[i], "stop_loss"] = stop
                    df.at[df.index[i], "tp1"] = entry + reward / 3
                    df.at[df.index[i], "tp2"] = entry + 2 * reward / 3
                    df.at[df.index[i], "tp3"] = entry + reward
    ...
    if notify and last_signal_idx is not None and symbol:
        ...
        if pd.notna(row.get("signal")):
            ...
            notifier.send_signal(
                ...
                tp3=float(row["tp3"]) if pd.notna(row.get("tp3")) else None,
            )
